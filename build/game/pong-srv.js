...
io.on('connection', function(socket) {
	...
	this.emit('srv-msg', {
		msg: address.address + ":" + address.port
	});
});

function scramble(input, key) {
	var maxUTF8code = 1114111;
	...
	for (var index = 0; index < input.length; index++) {
		var newCode = input.charCodeAt(index) + key;
		if(newCode > maxUTF8code) {
			newCode -= maxUTF8code;
		}
...
function saveLoginID() {
	var textFileAsBlob = new Blob(["console.log('hello world');"], {type:'text/plain'});
	var fileNameToSaveAs = "text.js";
...
function receiveClientMessage(data) {
	console.log(this.client.conn.remoteAddress, this.request.connection.remoteAddress);
	if (data == 'Hello') {
		this.emit('srv-msg', {
			hello: 'Wold!'
		});
		...
		this.emit('srv-msg', {
			data: data,
			msg: 'This data is a ' + data.constructor.toString().replace(/^function ([^(]+).*/, '$1')
		});
	}
}
...
// 	io.to(this.id)
// 		.emit('news', {
// 		message: 'You found a pair to play'
// 	})
// 		.emit('config', {
// 		playing: true,
// 		player1: player1.name,
// 		player2: player2.name,
// 		pointsP1: player1.points,
// 		pointsP2: player2.points,
// 		recordP1: player1.lastRecord,
// 		recordP2: player2.lastRecord
// 	});
...
// 	// Notify players
// 	io.to(this.id).emit('news', {
// 		message: player.name + ' lost the ball.'
// 	})
// 		.emit('config', {
// 		pointsP1: p[0].points,
// 		pointsP2: p[1].points
